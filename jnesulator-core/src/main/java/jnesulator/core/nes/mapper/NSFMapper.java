...
public class NSFMapper extends BaseMapper {
	...
	private int sndchip;
	boolean vrc6 = false, vrc7 = false, mmc5 = false, n163 = false, s5b = false, hasInitSound = false, fds = false;
		...
		if (scanline == 240) {
			...
			if (getNES().getCPU().PC != 0xFFFB) {
				...
				if (unfinishedcounter < time) {
					++unfinishedcounter;
					System.err.println("Init routine hasn't returned in " + unfinishedcounter + " frames");
					return;
	...
	private void setBanks() {
		for (int i = 0; i < prg_map.length; ++i) {
			...
			if ((prg_map[i]) >= prg.length) {
				...
				prg_map[i] %= prg.length; // probably a bad idea in general
											// though
				...
				// size doesn't need to be a power of 2 and you can start
				// loading
