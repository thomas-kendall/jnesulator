...
public class MMC5Mapper extends BaseMapper {
		...
		if (addr >= 0x8000) {
			...
			if (prgMode == 0 || ((prgMode == 1) && (addr >= 0xc000 || romHere[1]))
					|| ((prgMode == 2) && ((addr >= 0xe000 || (addr >= 0xc000 && romHere[2]) || romHere[1]))
							|| ((prgMode == 3) && (addr >= 0xe000 || (addr >= 0xc000 && romHere[2])
									|| (addr >= 0xa000 && romHere[1]) || romHere[0])))) {
	...
	public void cartWrite(int addr,  int data) {
		if (addr < 0x5c00) {
			...
			case 0x5107:
				// fill attribute
				Arrays.fill(fillnt, 32 * 30, fillnt.length,
						data & 0x3 + (data & 3) << 2 + (data & 3) << 4 + (data & 3) << 6);
				break;
			case 0x5113:
				// PRG RAM register
				wrambank = data & 7;
			...
			//// System.err.println("RAM write to 0xA000 area " +
			//// utils.hex(addr) + " " + prgbank);
	...
	public int ppuRead(int addr) {
		...
		// well, as it turns out in the real NES, the MMC5 can in fact see
		// everything
		...
		if (addr < 0x2000) {
			...
			if (prevfetch == prevprevfetch && prevprevfetch == addr) {
				...
				// unfortunately I don't know how the MMC5 resets the counter
				// when PPU is off yet
				incScanline();
				exlatch = 0;
			}
