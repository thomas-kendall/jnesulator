...
public class VRC7SoundChip implements IExpansionSoundChip {
	...
	// this is the cut-down version from the vrc7. Only 6 channels, no
	// percussion.
	...
	private int[][] instdata = { // instrument parameters
			...
			{ 0x03, 0x21, 0x05, 0x06, 0xB8, 0x82, 0x42, 0x27 }, { 0x13, 0x41, 0x13, 0x0D, 0xD8, 0xD6, 0x23, 0x12 },
			{ 0x31, 0x11, 0x08, 0x08, 0xFA, 0x9A, 0x22, 0x02 }, { 0x31, 0x61, 0x18, 0x07, 0x78, 0x64, 0x30, 0x27 },
			{ 0x22, 0x21, 0x1E, 0x06, 0xF0, 0x76, 0x08, 0x28 }, { 0x02, 0x01, 0x06, 0x00, 0xF0, 0xF2, 0x03, 0xF5 },
			{ 0x21, 0x61, 0x1D, 0x07, 0x82, 0x81, 0x16, 0x07 }, { 0x23, 0x21, 0x1A, 0x17, 0xCF, 0x72, 0x25, 0x17 },
			{ 0x15, 0x11, 0x25, 0x00, 0x4F, 0x71, 0x00, 0x11 }, { 0x85, 0x01, 0x12, 0x0F, 0x99, 0xA2, 0x40, 0x02 },
			{ 0x07, 0xC1, 0x69, 0x07, 0xF3, 0xF5, 0xA7, 0x12 }, { 0x71, 0x23, 0x0D, 0x06, 0x66, 0x75, 0x23, 0x16 },
			{ 0x01, 0x02, 0xD3, 0x05, 0xA3, 0x92, 0xF7, 0x52 }, { 0x61, 0x63, 0x0C, 0x00, 0x94, 0xAF, 0x34, 0x06 },
	...
	public void clock(int cycle) {
		 ...
		 * problem if anyone wanted to use it on a PAL system real chip takes 72
		 * cycles to advance through all ch (49716 hz) I update all ch every 36
		 * NES clocks (about the same speed) but I do the carrier and the
		 * modulator in the same cycle instead of separately
		 */
	...
	private int exp(int val) {
		// perform e^x function on 13 bit fp output value using the hardware
		// table on the chip
		...
		int exponent = (-val) >> 8;
		// int a = (int) Math.scalb(mantissa + 1024, exponent) * s; //correct
		// but slow
		return ((((mantissa + 1024) >> (-exponent)))) * s; // not correct for
															// negative #s
	}
	...
	private void operate() {
		...
		// envelopes
		int modEnvelope = setenvelope(inst, modenv_state, modenv_vol, ch, false) << 2;
		int carEnvelope = setenvelope(inst, carenv_state, carenv_vol, ch, true) << 2;
		// key scaling
	...
	private int setenvelope(int[] instrument, EnvState[] state, int[] vol, int ch, boolean isCarrier) {
		...
		// mess)
		/*
		 * Most of these constants were computed backwards from a table in a
		 * badly translated YM2413 technical manual (that was given in ms to
		 * decay 40db) and are thus only approximate. The key scaling stuff is
		 * similarly just a best guess. I suspect it's a 23 bit int value and
		 * each envelope update changes at least 1 LSB
		 */
		...
		case CUTOFF:
			if (vol[ch] < ZEROVOL) {
				 ...
				 * before the new note starts its attack run. need to listen
				 * really hard to the HW recordings and tune this by ear
				 */
			} else {
				vol[ch] = ZEROVOL;
				if (key[ch]) {
					/*
					 * the programmer's manual suggests that sound has to decay
					 * back to zero volume when keyed on before the attack
					 * happens, but other references don't say this
					 */
					state[ch] = EnvState.ATTACK;
					phase[ch] = 0;
					// reset phase to avoid popping? can't tell if the chip does
					// this.
				}
			}
			break;
		...
		case RELEASE:
			...
			// What makes this especially confusing is that the sustain bit in
			// the patch
	...
	public void write(int register, int data) {
		...
		case 0x25: // ??stooof
			...
			// o: Octave Select
			// t: Channel keying on/off (key on = note starts, key off: note
			// decays).
		...
		case 0x35: // top 4 bits instrument number, bottom 4 volume
			...
			break;
		default:
			// System.err.println(hex(register) + " doesn't exist " +
			// hex(data));
		}
	}
}
