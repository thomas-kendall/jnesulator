...
public class Namco163SoundChip implements IExpansionSoundChip {
	 ...
	 * increases, the sample rate decreases. Since the original chip only
	 * outputs one channel at a time and cycles between them, using all 8
	...
	private void output() {
		...
		// this low pass filter is here to reduce noise in games using 8
		// channels
		// while still letting me output 1 after the other like the real chip
		// does
		sample += lpaccum;
		lpaccum -= sample * (1 / 16.);
	}
