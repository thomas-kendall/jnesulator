...
public class FDSSoundChip implements IExpansionSoundChip {
	// emulates the wavetable channel in the FDS 2C33 sound chip
	// (does anything ever read back from these registers? they aren't all
	// write-only)
	...
	int waveAddr, waveOut, waveAccum; // 16 bits
	...
	int volEnvSpeed, modEnvSpeed, envClockMultiplier = 0xe8;// bios sets it
															// thus?
	int pitch; // 12 bits
	// modulation
	boolean modDisable;
	...
	private void CalculateModulator() {
		...
		// 1. multiply counter by gain, lose lowest 4 bits of result but "round"
		// in a strange way
		...
		// 3. multiply result by pitch, then round to nearest while dropping 6
		// bits
		temp = pitch * temp;
		remainder = temp & 0x3F;
		temp >>= 6;
	...
	public void write(int register, int data) {
		...
		if (regEnable) {
			if (register >= 0x4040 && register <= 0x407f) {
				...
				volEnvDisable = ((data & (Utils.BIT7)) != 0); // ON when it's
																// FALSE
