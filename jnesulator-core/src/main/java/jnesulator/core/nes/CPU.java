...
public class CPU {
	...
	public int PC;
	private boolean carryFlag = false, zeroFlag = false, interruptsDisabled = true, decimalModeFlag = false;
	private boolean overflowFlag = false, negativeFlag = false, previntflag = false, nmi = false, prevnmi = false,
			logging = false;
	...
	private void adc(final int addr) {
		...
		// set overflow flag
		overflowFlag = (((A ^ value) & 0x80) == 0) && (((A ^ result) & 0x80) != 0);
		A = result & 0xff;
	...
	/*
	 * really every instruction should be reading from or writing something to
	 * memory every cycle. Even when all that's happening that cycle is the
	 * processor updating state internally Fetching the next opcode cn overlap
	 * with last cycle of prev instruction if that last cycle is purely
	 * internal. but since the second cycle of all instructions (even single
	 * byte ones) is reading the nest byte after the PC, the fastest we can do
	 * even a single byte NOP instruction is still 2 cycles. that's where the
	 * dummy reads+writes come from. how to represent this in the smallest space
	 * possible? probably the way they did it on the real chip: using a PLA that
	 * does certain things conditionally based on bits of the current opcode and
	 * the current cycle (up to 7 i suppose) Bisqwit did some really nifty
	 * template stuff with his C==10 emu that I can't match.
	 */
	...
	public final void runcycle(final int scanline, final int pixel) {
		...
		if (idle && idleLoopSkip) {
			cycles += 3; // not accurate should depend on type of instr we skip
							// decoding
			return;
		}
		...
		if (logging) {
			...
			// also, logging *might* trigger side effects if logging while
			// executing
			// code from i/o registers (reading twice). So we don't want to do
			// it always.
