	...
	public void parseHeader() throws BadMapperException {
		...
		if (header[0] == 'N' && header[1] == 'E' && header[2] == 'S' && header[3] == 0x1A) {
			...
			if (((header[7] >> 2) & 3) == 2) {
				...
				prgsize = Math.min(therom.length - 16, 16384 * (header[4] + ((header[9] & 15) << 8)));
				if (prgsize == 0) {
					throw new BadMapperException("No PRG ROM size in header");
				}
				chrsize = Math.min(therom.length - 16 - prgsize, 8192 * (header[5] + ((header[9] >> 4) << 8)));
				...
				if (prgsize == 0) {
					...
					// and someone ELSE made this zero for an 8k PRG dump
					// (no-intro)
					// so if anyone gets this error make some heuristics to fix
					// it.
			...
			prgoff = 0;
			chroff = 0 + prgsize;
		} else if (('N' == header[0]) && ('E' == header[1]) && ('S' == header[2]) && ('M' == header[3])
				&& (0x1a == header[4])) {
			// nsf file
			mappertype = -1;
